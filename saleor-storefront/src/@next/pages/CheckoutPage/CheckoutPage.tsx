...
const handlePaymentConfirm = async () => {
  ...
  if (
    ...
    payment?.gateway !== "tappayment.gosell"
  ) {
    const paymentStepLink = steps.find(
      step => step.step === CheckoutStep.Payment
    )?.link;
  ...
  if (
    ...
    tappayConfirmationStatus.includes(querystring.status) 
  ) {
    ...
    setPaymentGatewayErrors([
      {
        message: translateAdyenConfirmationError(querystring.resultCode, intl)
      }
    ]);
    const paymentStepLink = steps.find(
      step => step.step === CheckoutStep.Payment
